...
function resultRow(p){return `<a class="search-result" href="${href(p)}"><img src="${p.image}" alt=""><span>${escapeHtml(p.name)}</span><small>${money(p.price)}</small></a>`}
...
function restartHero(){clearTimeout(heroTimer);if(!paused&&!document.hidden&&!$$("dialog[open]").length)heroTimer=setTimeout(()=>showSlide(slide+1),7000)}
...
$("[data-hero-prev]").onclick=()=>showSlide(slide-1);$("[data-hero-next]").onclick=()=>showSlide(slide+1);$$("[data-slide]").forEach(b=>b.onclick=()=>showSlide(Number(b.dataset.slide)));$("[data-hero-pause]").onclick=()=>pauseHero(!paused);
...
const stories=[
 ["Cherry 33","Спелая вишня, яркое начало и тёплый след.","assets/media/fluide-editorial-stilllife-v1.jpg","catalog.html","Открыть коллекцию"],
 ...
 ["Лёгкое прикосновение","Парфюмированные мисты для ежедневного ритуала.","assets/media/category-care-studio-v1.png","#contacts","Узнать больше"],
 ["Атмосфера дома","Тёплый свет, любимое место и аромат Cashmere.","assets/media/category-home-studio-v1.png","#contacts","Узнать больше"],
...
const questions=[
 {title:"Какие ноты вам ближе?",options:[["Свежие","Цитрусы, зелень, прохладный воздух","бергамот лимон грейпфрут"],["Мягкие","Чистота, мускус, нежные цветы","мускус ирис"],["Тёплые","Дерево, пряности, амбра","кардамон сандал кедр"],["Сочные","Ягоды, фрукты, сладкие акценты","малина ананас яблоко"]]},
 {title:"Для какого настроения?",options:[["На каждый день","Лёгкий, спокойный спутник","ирис бергамот"],["Для особого момента","Выразительный, запоминающийся","кардамон ананас"],["По настроению","Люблю менять и пробовать","мускус яблоко"]]},
 {title:"Для кого выбираете?",options:[["Для неё","","women"],["Для него","","men"],["Без границ","","unisex"]]}
];let answers=[];
