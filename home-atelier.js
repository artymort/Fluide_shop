...
function updateCart(){save("fluide-cart",cart);const quantity=cart.reduce((n,p)=>n+(Number(p.quantity)||1),0);const badge=$("#cart-count");badge.textContent=quantity||"";badge.hidden=quantity===0;renderHomeCart()}
function renderHomeCart(){
 const body=$(".home-cart-items");if(!body)return;
 const detailed=cart.map((row,index)=>({row,index,product:row.product||products.find(product=>product.id===row.id)})).filter(item=>item.product);
 const promotion=window.FluidePromotions?.calculate(cart)||{total:detailed.reduce((sum,item)=>sum+(Number(item.product.price)||0)*(Number(item.row.quantity)||0),0),discount:0,giftItemCounts:{}};
 if(!detailed.length){body.innerHTML='<div class="cart-empty"><div class="cart-empty-content"><p>В корзине пока пусто</p><small>Добавьте аромат из коллекции</small><a class="cart-continue" href="catalog.html"><span>Продолжить выбор</span><svg aria-hidden="true"><use href="#move-right"/></svg></a></div></div>';return}
 body.innerHTML=`${window.FluidePromotions?.bannerMarkup()||""}${detailed.map(({row,index,product})=>{const quantity=Number(row.quantity)||1;const giftQuantity=promotion.giftItemCounts?.[String(row.id)]||0;const gross=(Number(product.price)||0)*quantity;const paid=(Number(product.price)||0)*(quantity-giftQuantity);return `<div class="cart-row"><img src="${escapeHtml(product.image||"assets/brand/logo-blue.svg")}" alt="${escapeHtml(product.name||"Товар FLUIDE")}"><div class="cart-row-copy"><h3>${escapeHtml(product.name||"Товар FLUIDE")}</h3>${giftQuantity?'<span class="cart-gift-label">Подарок по акции 3+1</span>':""}</div><button type="button" data-remove="${index}" aria-label="Удалить ${escapeHtml(product.name||"товар")}"><svg><use href="#x"/></svg></button><div class="cart-quantity"><button type="button" data-quantity="${index}" data-delta="-1" aria-label="Уменьшить количество" ${quantity<=1?"disabled":""}><svg><use href="#minus"/></svg></button><span>${quantity}</span><button type="button" data-quantity="${index}" data-delta="1" aria-label="Увеличить количество"><svg><use href="#plus"/></svg></button></div><div class="cart-row-total">${giftQuantity?`<del>${money(gross)}</del><strong>${money(paid)}</strong>`:`<strong>${money(gross)}</strong>`}</div></div>`}).join("")}<a class="cart-continue" href="catalog.html"><span>Продолжить выбор</span><svg aria-hidden="true"><use href="#move-right"/></svg></a>${window.FluidePromotions?.totalsMarkup(promotion,money)||`<section class="cart-order-summary"><div class="cart-order-total"><span>Итого</span><strong>${money(promotion.total)}</strong></div><button class="cart-summary-checkout checkout-button" type="button">Оформить заказ</button></section>`}`;
}
function openHomeCart(){renderHomeCart();if($("#panel").open)$("#panel").close();$(".home-cart-drawer").classList.add("is-open");$(".home-cart-drawer").setAttribute("aria-hidden","false");$(".home-cart-backdrop").classList.add("is-open");document.body.classList.add("is-locked")}
function closeHomeCart(){$(".home-cart-drawer").classList.remove("is-open");$(".home-cart-drawer").setAttribute("aria-hidden","true");$(".home-cart-backdrop").classList.remove("is-open");document.body.classList.remove("is-locked")}
...
function openPanel(mode,title){closeHomeCart();panelMode=mode;$("#panel-title").textContent=title||({login:"Личный кабинет",search:"Найти аромат",favorites:"Избранное",menu:"FLUIDE"}[mode]||mode);renderPanel();if(!$("#panel").open)$("#panel").showModal();if(mode==="search")$("#home-search").focus()}
function renderPanel(){
 ...
 if(panelMode==="cart"){
 const promotion=window.FluidePromotions?.calculate(cart)||{total:0,discount:0,giftItemIds:[],giftItemCounts:{}};let gross=0;body.innerHTML=(cart.length?(window.FluidePromotions?.bannerMarkup()||""):"")+cart.map((row,i)=>{const p=row.product||products.find(p=>p.id===row.id);if(!p)return "";gross+=(Number(p.price)||0)*row.quantity;if(!window.FluidePromotions)promotion.total=gross;return `<div class="cart-row"><img src="${escapeHtml(p.image)}" alt=""><div class="cart-row-copy"><h3>${escapeHtml(p.name)}</h3>${promotion.giftItemCounts?.[String(row.id)]?`<span class="cart-gift-label">Подарок по акции 3+1</span>`:""}</div><button type="button" data-remove="${i}" aria-label="Удалить ${escapeHtml(p.name)}"><svg><use href="#x"/></svg></button><div class="cart-quantity"><button type="button" data-quantity="${i}" data-delta="-1" aria-label="Уменьшить количество" ${Number(row.quantity)<=1?"disabled":""}><svg><use href="#minus"/></svg></button><span>${row.quantity}</span><button type="button" data-quantity="${i}" data-delta="1" aria-label="Увеличить количество"><svg><use href="#plus"/></svg></button></div><div class="cart-row-total">${promotion.giftItemCounts?.[String(row.id)]?`<del>${money((Number(p.price)||0)*Number(row.quantity))}</del><strong>${money((Number(p.price)||0)*(Number(row.quantity)-promotion.giftItemCounts[String(row.id)]))}</strong>`:`<strong>${money((Number(p.price)||0)*Number(row.quantity))}</strong>`}</div></div>`}).join("")||(cart.length?'':'<p>Здесь будут ваши ароматы.</p>');
 ...
 if(cart.length)body.innerHTML+=window.FluidePromotions?.totalsMarkup(promotion,money)||`<div class="cart-total"><span>Итого</span><span>${money(promotion.total)}</span></div><button class="pill dark checkout-button" type="button" data-checkout>Оформить заказ</button>`;
...
document.addEventListener("click",event=>{
 ...
 if(b.dataset.open){if(b.dataset.open==="cart")openHomeCart();else openPanel(b.dataset.open)}
 ...
 if(b.hasAttribute("data-quantity")){const row=cart[Number(b.dataset.quantity)];if(row){row.quantity+=Number(b.dataset.delta);cart=cart.filter(x=>x.quantity>0);updateCart();if(panelMode==="cart"&&$("#panel").open)renderPanel()}}
 if(b.hasAttribute("data-remove")){cart.splice(Number(b.dataset.remove),1);updateCart();if(panelMode==="cart"&&$("#panel").open)renderPanel()}
 if(b.hasAttribute("data-home-cart-close"))closeHomeCart();
 if(b.dataset.info){openPanel("info",{delivery:"Доставка и оплата",contacts:"Связаться с FLUIDE",privacy:"Персональные данные"}[b.dataset.info]);$("#panel-body").innerHTML={delivery:'<p>Заказ можно оформить с доставкой по России или самовывозом во Владимире.</p><p>Стоимость доставки и способ оплаты менеджер подтвердит после оформления.</p>',contacts:'<p>FLUIDE Atelier — парфюмерный бренд из Владимира.</p><p>Контакты и часы работы будут добавлены после подтверждения командой бренда.</p><a class="pill outline" href="about.html">О нашем пространстве ↗</a>',privacy:'<p>Корзина, избранное и выбор cookie сохраняются локально в вашем браузере.</p><p>Контакты и адрес из формы используются только для обработки и доставки заказа.</p>'}[b.dataset.info]}
...
$(".home-cart-backdrop").addEventListener("click",closeHomeCart);
document.addEventListener("keydown",event=>{if(event.key==="Escape"&&$(".home-cart-drawer").classList.contains("is-open"))closeHomeCart()});
